...
class DataAnalysis:
    ...
    def data_analysis(self,):
        ...
        Utilities.generate_heatmap(all_results)
        ...
        if self.cross_validate:
            ...
            # for backtest_dataframe_key in backtest_dataframe_keys:
            #     cross_validation_result = CrossValidator.generate_tasks_from_results(backtest_dataframe_key, filtered_results, self.minimum_sharpe, self.kwargs)
            #     merged_df = pd.merge(pd.DataFrame(result_data_list).sort_values(by='sharpe', ascending=False).reset_index(drop=True), pd.DataFrame(cross_validation_result), on='strategy', how='inner')
            #     cols = [col for col in merged_df.columns if col != 'strategy'] + ['strategy']
            #     merged_df = merged_df[cols]
            #     pass
