...
class DataAnalysis:
    ...
    def __init__(self, kwargs):
        ...
        self.generate_heatmap = kwargs['generate_heatmap']
    ...
    def data_analysis(self,):
        ...
        if self.walk_forward         : WalkForwardAnalysis(self.kwargs['asset_currency'], self.kwargs).run_walk_forward_analysis()
        if self.parameter_plateau    : ParameterPlateau(self.kwargs['asset_currency'], self.kwargs).optimize_parameters()
        if self.cross_validate       : CrossValidator(self.kwargs['asset_currency'], self.kwargs).process_backtest_results()
        if self.generate_heatmap     : HeatmapGenerator(self.kwargs['asset_currency'], self.kwargs).generate_all_heatmaps(all_results, self.kwargs['show_heatmap'], self.kwargs['target_metric'])
        if self.generate_equity_curve: EquityCurveGenerator(self.kwargs['asset_currency'], self.kwargs).generate_equity_curves(self.kwargs['show_equity_curve'])